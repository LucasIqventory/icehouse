...
function toSubmitContactRequest(values: ContactFormValues, source: ContactSource): SubmitContactRequest {
  ...
  return {
    ...
    // The backend contract requires this exact hyphenated key.
    ...(trimmedHeardFrom ? { 'heard-from': trimmedHeardFrom } : {}),
  };
}
  ...
  try {
    ...
    if (!response.ok) {
      return {
        ...
        message: extractMessage(body, 'Unable to submit your message right now. Please try again.'),
      };
    }
    ...
    return {
      ok: true,
      status: response.status,
      message: extractMessage(body, 'Thanks for reaching out. We will be in touch soon.'),
    };
  } catch {
    return {
      ok: false,
