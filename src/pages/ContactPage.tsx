...
export default function ContactPage() {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>): Promise<void> {
    ...
    if (!form.checkValidity() || hasFieldErrors(nextFieldErrors)) {
    ...
    setStatus({ tone: 'idle', message: 'Sending message...' });
  ...
  return (
    <main id="main">
      <section className="hero hero-contact" aria-label="Contact Ice House">
        <div className="hero-media" aria-hidden="true">
          <img
            src="/optimized/media/tailgatehero.avif"
            alt=""
            decoding="async"
            fetchPriority="high"
          />
        </div>
        <div className="section-shell hero-content product-hero-copy">
          <p className="eyebrow">Let&apos;s Talk Game Day.</p>
          <h1>CONTACT ICE HOUSE</h1>
          <p className="hero-copy">
            Contact us to learn more about available models, customization options, and ordering.
          </p>
        </div>
      </section>

      <section className="section-band frost-band contact-section">
        <div className="section-shell contact-grid">
            ...
            <div className="form-grid">
              ...
              <label className="full-field">
                ...
                <textarea
                  name="message"
                  rows={3}
                  placeholder="Where will it take you? Who's coming along?"
              ...
              {fieldErrors.name ? (
                <p className="field-error-message full-field" id="contact-name-error">
                  {fieldErrors.name}
                </p>
              ) : null}
              {fieldErrors.email ? (
                <p className="field-error-message full-field" id="contact-email-error">
                  {fieldErrors.email}
                </p>
              ) : null}
            </div>

            <button className="ice-button primary" type="submit" disabled={isSubmitting}>
              {isSubmitting ? 'SENDING...' : 'SEND MESSAGE'}
            </button>
            <p
              className={`form-status${status.tone === 'success' ? ' is-success' : ''}${status.tone === 'error' ? ' is-error' : ''}`}
              role="status"
              aria-live="polite"
            >
              {status.message}
          ...
          <div className="contact-side-rail">
            <aside className="company-panel" aria-label="Company information">
              <p className="eyebrow">Company Information</p>
              <h2>ICE HOUSE TAILGATING</h2>
              <p>A Division of Crescent Moon Vans, LLC</p>
              <address>
                995 Holland Ave
                <br />
                Cayce, SC 29033
              ...
              <figure className="contact-logo-mark">
                <img
                  src="/IceHouselogos/Color%20Igloo%20+%20White%20Tag.png"
                  alt="Ice House Tailgating logo"
                  loading="lazy"
                  decoding="async"
                />
              </figure>
            ...
            <div className="contact-cmv-card">
              <p className="cmv-eyebrow">Sister Brand</p>
              <h3 className="contact-cmv-title">
                More interested in custom and prebuilt vans?
              </h3>
              <p className="contact-cmv-copy">
                Crescent Moon Vans builds custom campervans, mobile business builds, shell builds,
                and ready-to-go floorplans.
              </p>
              <p className="cmv-hand">do whatever you want, wherever you want</p>
              <a
                className="cmv-button"
                href="https://crescentmoonvans.com/"
                target="_blank"
                rel="noopener noreferrer"
              >
                Visit Crescent Moon Vans
              </a>
            </div>
          </div>
        </div>
